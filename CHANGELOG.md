...
- Prompt 02 壓縮成 7 節單一核心 meta instruction，不再把平台入口或工具適配寫成核心職責。
- 發布後未升版本補強：Prompt 02 現在更明確承認已由使用者或平台採用、且有明示責任範圍的 workflow、skill 或 governance tool；Agent Handoff Kit、CER 工作法等工具在責任範圍內的啟動、狀態保存、讀回、收尾、固定輸出與阻塞回報不可被「最小化」省略，也不可因此推定越權操作。
...
- Prompt 02 的長期指令整理規則改得更通用：先保留會影響 AI 行為、安全、權限、資料處理、事實核對、交付與驗收的要求，再把只供理解的背景、例子、宣傳、測試結果與教學移到參考資料。
...
- 繁中與英文 Prompt 02 現在明確處理工具通道、sandbox、patch、測試或讀取命令無回應的情況：先確認沒有半寫入，再只用同等安全、已授權、可審計的官方工具通道重試一次；仍失敗時才以已核實狀態和最小人工動作收口。
- Prompt 02 的優先序改為先守住事實可驗收、安全、使用者目標與明示範圍，再做最小足夠改動；不得因根因治理、完整交付、持久化或同步，自動擴張到未要求的外部、公開、不可逆、跨表面或長期治理工作。
...
- 繁中與英文 Prompt 02 現在明確承認 Agent Handoff Kit 在其責任範圍內負責啟動、交接、品質檢查、升級與發布流程，避免全域指令與專案交接工具互相搶真源。
...
- 繁中與英文 Prompt 02 在核心資料、能力或安全條件不足而受阻時，會以白話交代缺口及影響、已完成的安全／唯讀核實，並提出不多於三項可行下一步及建議；需要使用者處理時，只索取最小資料或提供可複製確認句。
