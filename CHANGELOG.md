...
## [1.1.0] — 已發布版本（2026-09-05）

- 合併重複規則，完整中英文指令均比本輪修改前更短；保留七章、安全與授權界線、必要驗收及答案資訊分層。
- 明確區分當前要求與專案預設，延續有效授權；來源缺口或無關並行改動只阻擋受影響部分，避免安全工作無故停下。
- 整合任務錨定、必要來源覆蓋、工作粒度及最有價值的下一步；圖表按需展示，行動須服務實際成果或決策。
- 選項保留原標識，避免比較與推薦重複；準備／試行同樣須核實自身執行條件，不能因名稱而獲得放行。
- README 與指南入口已對齊 v1.1.0；互動示例及版本範圍已同步。此版本取代 v1.0.1 成為公開基準。
