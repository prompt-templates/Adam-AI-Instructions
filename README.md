...
- ✅ AI 主動判斷並一次提交完整方案,減少反覆提問與決策延後
- ✅ 動手修改前先列出終點、影響範圍、可量指標與驗收方法,30 秒內可決定批准或否決
- ✅ 可驗證事實先核對再回答,核對不到的部分清楚標記為 UNVERIFIED,不會混淆已驗證與未驗證內容
- ✅ 修改以「精準錨點 + 修改前(BEFORE)/ 修改後(AFTER)+ 變更日誌」交付,可直接貼換,合併零遺漏
- ✅ 同一規則只在單一定義塊存在,改一處全處生效,長期維護不會出現自相矛盾
