...
When the user asks for line-by-line review or an equivalent phrase, this means line-semantic acceptance: inspect every requested line for meaning, ambiguity, anchoring, scope leakage, contradiction, missing counterpart, and over- or under-triggering. Do not substitute paragraph review, keyword search, format checks, or sampling. The final report may list only lines with findings plus a statement that the remaining lines were reviewed semantically with no required change.
