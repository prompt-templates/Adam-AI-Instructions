...
root README 累積多輪變更後仍含三類漂移：第一節「全圖優先規則」五區段名為舊版；第一節「UNVERIFIED」「BEFORE/AFTER」英文措辭與 prompt.md 中文不一致；第三節目錄結構樹只列 prompt 01；第五節「01 vs 02 怎麼選」尾句仍寫「第一至二十節」（v0.7.1 命名同步漏網）。

### 修正

- `README.md` 第一節「本資料庫內容」：全圖優先規則描述替換為實際五區段名（終點畫面、交付物、可量指標、驗收測試、目標連結）+ 收尾句；UNVERIFIED 改為「未核實／未適用」中文二分；BEFORE/AFTER 改為「修改前／修改後」；新增 3 條好處（先讀後判、治理改動先審核、創作豁免）。
- `README.md` 第三節「目錄結構」：樹圖完整重寫，列出 prompt 01／02 三件套、docs/releases/、docs/experiments/、CHANGELOG.md、AGENTS.md、LICENSE。
- `README.md` 第五節「Prompt 索引」：「02 第一至二十節...」改為「02 第一至十二節 + 第十三節機密處理 + 第十四節 Windows 桌面附加」。
- `README.md` 排版：移除第四節結尾與 spotlight box 之間多餘嘅 horizontal rule。

### 治理層教訓

多輪 hotfix 後 root README 易積漂移：第一節「好處清單」與第三節「目錄結構樹」屬「敘事性介紹」，每次都未被批量替換腳本掃描到。「五區段名稱」屬規格性事實，文檔中描述五區段時必須用實際名稱（終點畫面／交付物／可量指標／驗收測試／目標連結），不可用泛化說明替代。
...
v0.7.0 經實測勝出但對普通訪客而言「為何 B 勝出」嘅故事埋藏於實驗報告深處；root README 索引表亦未突出 v0.7.0 嘅勝出地位。本版本以一般讀者立場改造兩處入口頁，加入功能性 emoji 視覺定位、🏆 一句結論卡、spotlight box，讓首次訪客 30 秒內掌握「v0.7.0 — 12-節結構整合版」嘅勝出依據與安裝路徑。

### 修正

- `README.md`：第四節與第五節之間插入 🏆 spotlight box（一句結論 + 量化指標 + 兩條直接行動連結 + 實驗報告連結）；第六節最近更新表 v0.7.0 行加 🏆 prefix + 粗體版本名。
- `docs/experiments/2026-05-23-pruning-experiment/README.md`：首屏加 🏆 一句結論 callout box；各節改用功能性 emoji 標頭（🎯 為何做 / 🧬 設計 / 📋 案例 / 📊 維度 / 🏆 結果 / ✅ 覆蓋率 / 📈 排名 / 💡 為何勝出 / 🎁 實用價值 / 📁 索引 / 📚 來源）；策略對比表加 ⭐ 標 B 中度行；排名表加 🏆 🥈 🥉；檔案索引 v_b_moderate.md 行加 🏆 + 「**已採用**」粗體。

### 設計原則

按用戶偏好「功能性 emoji 只作視覺定位，每枚單一語意，只放標題或項目開首，不插入正文句中，不作裝飾」，新引入嘅功能性 emoji 全部單一語意：🏆 勝出標記、⭐ 焦點強調、🥈 🥉 排名輔助、🔎 重點問題、✅ 通過項、📊 📈 📉 數據指標、🎯 目標、🧬 🧪 📋 實驗工具、💡 為何成立、🎁 用戶價值、📁 檔案索引、📚 文件來源、🚀 行動連結、👉 即時動作指引。

### 對應的用戶感受變化

- 首次訪客開 root README，第四節與第五節之間即見 🏆 spotlight box，30 秒內掌握 v0.7.0 嘅勝出地位、量化指標、安裝路徑
- 第六節最近更新表中 v0.7.0 行視覺突出
- 訪 experiment 報告首屏即見「🏆 一句結論」，知道 B 勝出、勝出依據、即時試用連結，毋須讀畢全文
- 報告各節按功能性 emoji 視覺分組，眼球有焦點落腳

### 治理層教訓

每次重要 release 後須做「30 秒首屏測試」：想像首次訪客掃一眼，是否能即時掌握「乜嘢勝出、為何勝出、即時試用連結」三項。技術人員寫嘅 README 容易預設讀者已掌握 context；功能性 emoji 對非技術讀者導讀效益顯著，但前提是「每枚單一語意、只放標題或項目開首、不裝飾」。亂用 emoji 會反效果。已記入 memory feedback layer。
...
修補 v0.7.0 release 後發現的三項漏洞：root README 第五節 02 行未同步新節號；prompts/01 與 prompts/02 嘅互動指南 guide.html 仍含大量舊節號（SVG 標籤、h2 標題、正文引用）；勝出版本一直用實驗代號「中度瘦身版 B」對外溝通，非技術讀者難分辨。

### 修正

- `README.md` 第 109 行：第五節索引表 02 行「適用工具」欄「第二十二節 Windows 桌面附加為條件式生效」改為「第十四節 Windows 桌面附加為條件式生效」。第 127 行 v0.6.0 歷史紀錄保留原節號（屬發佈當時事實）。
- `prompts/02-claude-code-meta-instruction/guide.html`：SVG 節數標籤按 v0.7.0 結構整合版重編並更新節名（第一節 語言與語體／第四節 全圖優先規則／第八節 治理改動先審核／第九節 變更交付：補丁式／第十節 防漂移與用語紀律／第十二節 執行環境補充）；h2 標題（第十三節 機密處理／第十四節 執行環境特定附加）；正文／表格全部按新編號（第一至十二節／第十三、十四節／其餘 13 節／共 12 節）。
- `prompts/01-claude-cowork-meta-instruction/guide.html`：在 `<main>` 標籤前插入「v0.7.0 結構更新」提示 banner，告知讀者 prompt.md 已由 20 節整合為 12 節，並指向 2026-05-23 瘦身實驗報告。本指南按主題的「8 大組規則」分組仍完全對應新結構，可繼續使用。

### 命名規約

為勝出版本確立正式名稱：**v0.7.0 — 12-節結構整合版**（簡稱「12-節結構整合版」或「v0.7.0 結構整合版」）。同步更新 docs/experiments/ 主 README、comparison_report、v_b_moderate.md 標題、docs/releases/v0.7.0.md、CHANGELOG.md v0.7.0 條目。實驗報告中 A 保守／B 中度／C 激進對照表嘅欄頭保留原名（描述實驗策略幅度，非版本名）。

### 對應的用戶感受變化

- root README 第五節索引表 02 行嘅節號引用與 prompt 02 本身一致，按引用查即找到「第十四節」
- guide.html 兩檔所有節數標籤、h2、正文都與最新 prompt.md 一致，無誤導
- guide.html 01 頂部有 v0.7.0 結構更新提示 banner，知道 prompt.md 已升版而本指南仍可用
- release note、實驗報告、CHANGELOG 嘅版本名明確是「v0.7.0 — 12-節結構整合版」而非「B 版」

### 治理層教訓

- v0.7.0 release 前嘅批量節號替換腳本只覆蓋 prompts/ 下嘅 README，未覆蓋 root README 與 guide.html。屬「修改前最低閱讀覆蓋」未做足。應先列出全 repo 對舊節號的所有引用，再做替換。
- guide.html 是視覺指南，含 SVG 嵌入節數標籤，等同雙處同步源。長遠應由腳本根據 prompt.md 自動生成 SVG 節數標籤，或將節數從 SVG 標籤抽離為註腳。已記入 dev/SESSION_LOG.md 等下次重構處理。
- 「B 版」對外溝通漏洞屬命名規約缺失。實驗代號（A／B／C）應與正式版本名（v0.7.0）分開：前者描述策略幅度只在實驗內使用，後者用於對外。已在 v0.7.1 release note 第二節確立規約。
