...
- `prompts/01-claude-cowork-meta-instruction/prompt.md`：替換為 B 版 12 節結構。同質規則合併：第一節（語言＋語體）、第三節（核心原則＋骨架＋輸出控制）、第六節（歧義處理＋真源對齊）、第九節（補丁式＋深度修補）、第十節（防漂移＋用語紀律）、第十一節（計算＋格式硬規則）、第十二節（執行環境補充 19 條 → 12 條，子規則合併但保留全部語意）。廢止冗餘：第七節「對話層避免堆砌內部代碼」、第八節「短答優先」、第十九節「示例要通用」、第二十節第 13 條元宣告。
